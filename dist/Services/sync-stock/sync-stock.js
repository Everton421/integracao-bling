...
class SyncStock {
     ...
     * @returns
     */
    async postEstoque(idProdutobling, saldo, idDeposito, codigoProdutoSistema, data_estoque) {
    ...
    async getDeposit() {
        ...
        const produtoApi = new produto_api_repository_1.ProdutoApiRepository();
        const arrDeposito = resultDeposito.data.data;
        let id_bling;
        let objeDeposit;
        ...
        try {
            ...
            if (produtosEnviados.length > 0) {
                for (const data of produtosEnviados) {
                    const resultSaldo = await this.produto.buscaEstoqueReal(data.codigo_sistema, 1);
